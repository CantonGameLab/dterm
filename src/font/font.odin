...
glyphRasterize :: proc(font_h : mem.Handle, cp : rune) -> bool {
	...
	w, h, xoff, yoff : c.int
	stbtt.GetCodepointBitmapBox(&face.info, cp, face.scale, face.scale, &xoff, &yoff, &w, &h)
	if w == 0 || h == 0 {
	...
	return slotInsert(font_h, GlyphSlot {
		...
		w = u16(w), h = u16(h),
		xoff = f32(xoff) + sub_x, yoff = f32(yoff) + sub_y,
		advance = f32(advance) * face.scale,
		u0 = f32(x + ATLAS_PAD) / f32(font.atlas.width),
		v0 = f32(y + ATLAS_PAD) / f32(font.atlas.height),
		u1 = f32(x + ATLAS_PAD + w) / f32(font.atlas.width),
		v1 = f32(y + ATLAS_PAD + h) / f32(font.atlas.height),
	})
}
...
atlasGrow :: proc(font_h : mem.Handle) {
	...
	for i in 0 ..< len(font.slots) {
		...
		sub_x, sub_y : f32
		stbtt.MakeCodepointBitmapSubpixelPrefilter(&face.info, cast([^]byte)&a.pixels[row_start], c.int(slot.w), c.int(slot.h), c.int(a.width), face.scale, face.scale, 0, 0, c.int(font.antialias), c.int(font.antialias), &sub_x, &sub_y, slot.cp)
		slot.u0 = f32(x + ATLAS_PAD) / f32(a.width)
		slot.v0 = f32(y + ATLAS_PAD) / f32(a.height)
		slot.u1 = f32(x + ATLAS_PAD + slot.w) / f32(a.width)
		slot.v1 = f32(y + ATLAS_PAD + slot.h) / f32(a.height)
